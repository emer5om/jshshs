...
const AddUserAddressesModal = ({
  ...
  const initialCenter = { lat: latitude ?? 23.242, lng: longitude ?? 69.6669 };
  const zoomLevel = 16;
  ...
  const [is_default_checked, set_is_default_checked] = useState(
    isDefault == "0" ? false : true
  );
    ...
    try {
      ...
      if (addUserAddress.error) {
        ...
        if (!getAddress.error) {
          ...
          toast.error(getAddress.message);
        }
      }
    } catch (error) {
