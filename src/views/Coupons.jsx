...
const Coupons = () => {
    ...
    const validate_promo_codes = async (promo_code) => {
        try {
            const promoCodes = await validatePromoCodes({ branch_id, promo_code, final_total: cartStoreData.overall_amount })
            if (!promoCodes.error) {
                console.log(promoCodes.data)
                            ...
                            <Card sx={{
                                ...
                                <CardContent sx={{ minWidth: { md: "60%", sm: "50%" }, width: "100%" }} >
                                    ...
                                    <Box width={"100%"} gap={3} display={"flex"} alignItems={"start"} flexDirection={"column"} justifyContent={"space-between"}>
                                        ...
                                        <Box>
                                            <Typography textColor={"neutral.500"} fontSize={"sm"} fontWeight={"md"}>expiry Date:    {formatDate(item.end_date)}
                                            </Typography>
                                        </Box>
                                    </Box>
                                </CardContent>
                                ...
                                <CardContent orientation="vertical"
                                    ...
                                    <Typography textColor={"text.menuText"} fontSize={"lg"} fontWeight={"xl"} >
                                        {item.promo_code}
                                    </Typography>
                                    {/* {console.log(promo)} */}
                                    <Button
